...
#if defined(NDEBUG)
#error "ROCO cannot be compiled without assertions."
#endif
...
//const int nVersionSecondFork = 220000;
...
/** Fees smaller than this (in uroco) are considered zero fee (for relaying and mining)
...
CAmount GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
	  ...
	  else if (nHeight <= 25000 && nHeight >= 17501) {
        nSubsidy = 4.5 * COIN;
    }
	  else if (nHeight <= 32500 && nHeight >= 25001) {
        nSubsidy = 6 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, unsigned mnlevel, int64_t blockValue)
{
  ...
  if (nHeight <= Params().StartMNPaymentsBlock())
    ...
    else if (nHeight <= 115001 && nHeight >= 70001) {
        ret = blockValue / 100 * 85;  //85%
    }
    else if (nHeight >= 115001) {  // 90%
