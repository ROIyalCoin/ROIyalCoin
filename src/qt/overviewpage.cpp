...
void OverviewPage::updatBlockChainInfo()
{
 if (masternodeSync.IsBlockchainSynced())
 {
    ...
    int64_t BlockReward = GetBlockValue(chainActive.Height(), 0);
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeROCOAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeROCOAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeROCOAmount = strAnonymizeROCOAmount.remove(strAnonymizeROCOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeROCOAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeROCOAmount * COIN) nMaxToAnonymize = nAnonymizeROCOAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeROCOAmount * COIN) {
 //       ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
   //                                           .arg(strAnonymizeROCOAmount));
        strAnonymizeROCOAmount = strAnonymizeROCOAmount.remove(strAnonymizeROCOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeROCOAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
